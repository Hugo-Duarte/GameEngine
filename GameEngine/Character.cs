...
namespace GameEngine
{
    public class Character : GameObject
    {
        ...
        protected float deceleration = 1.2f; // The lower it is, the slower you slow dowm.
        protected float acceleration = 0.78f; // The lower it is, the slower you start moving.
        protected float maxSpeed = 5f;

        const float gravity = 1f;
        const float jumpVelocity = 16f; //Hom much we jump.
        ...
        public void UpdateMovement(List<GameObject> objects, Map map)
        {
            ...
            velocity.X = TendToZero(velocity.X, deceleration);
            if (applyGravity == false)
                velocity.Y = TendToZero(velocity.Y, deceleration);
        }
        ...
        protected virtual bool CheckCollisions(Map map, List<GameObject> objects, bool xAxis) 
        {
            ...
            {
                if (gameObject != this && gameObject.active == true && gameObject.collidable == true && gameObject.CheckCollision(futureBoundingBox) == true)
                    return true;
            }
